...
const OrderTracking = () => {
    ...
    return (
        <MobileLayout hideNav>
            {/* ── Header ─────────────────────────────── */}
            <header className="px-5 pt-12 pb-5 bg-white/80 backdrop-blur-md sticky top-0 z-50 border-b border-white/5 flex items-center justify-between">
                <button onClick={() => navigate(-1)} className="w-10 h-10 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-center active:scale-90 transition-transform">
                ...
                <button className="w-10 h-10 bg-white/[0.02] border border-white/5 rounded-xl flex items-center justify-center active:scale-90 transition-transform">
            ...
            <div className="p-5 space-y-6 pb-32">
                {/* ── Status Pulse ─────────────────────────────── */}
                <div className="bg-white/5 rounded-3xl border border-white/5 shadow-2xl p-6 relative overflow-hidden">
                    ...
                    <div className="relative flex justify-between px-2">
                        {/* Connector Line */}
                        <div className="absolute top-4 left-8 right-8 h-1 bg-white/[0.05] rounded-full z-0 overflow-hidden">
                            <motion.div
                                initial={{ width: '0%' }}
                            ...
                            return (
                                <div key={s.id} className="relative z-10 flex flex-col items-center">
                                    <div className={`w-9 h-9 rounded-xl border-4 transition-all duration-500 flex items-center justify-center ${isDone ? 'bg-brand border-white shadow-lg' :
                                        isActive ? 'bg-white/5 border-brand shadow-2xl shadow-black/50' :
                                            'bg-white/5 border-gray-50'
                                        }`}>
                                        {isDone ? (
                ...
                {order.status !== 'delivered' && (
                    <div className="bg-brand/5 rounded-3xl border-white/5 border-brand/20 p-6 relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="flex gap-2">
                                {order.items?.[0]?.fulfillment?.deliveryPin?.split('').map((digit, i) => (
                                    <div key={i} className="flex-1 h-14 bg-white/5 border-white/5 border-brand/10 rounded-2xl flex items-center justify-center">
                ...
                {order.items?.some(it => it.fulfillment?.agentId) && (
                    <div className="bg-white/5 rounded-3xl border border-white/5 shadow-soft p-5">
                        ...
                        <div className="flex items-center gap-4">
                            <div className="w-14 h-14 rounded-2xl bg-white/[0.05] border border-white/5 overflow-hidden">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="bg-white/5 rounded-2xl p-4 border border-black/5">
                ...
                <div className="space-y-4">
                    ...
                    {Object.entries(itemsByVendor).map(([vendorId, vendorGroup]) => (
                        <div key={vendorId} className="bg-white/5 rounded-3xl border border-white/5 overflow-hidden shadow-soft">
                            <div className="px-5 py-3 bg-white/[0.02]/50 border-b border-white/5 flex items-center justify-between">
                                <div className="flex items-center gap-2">
                            ...
                            <div className="divide-y divide-gray-50">
                                {vendorGroup.items.map((item, idx) => (
                                    <div key={idx} className="p-4 flex gap-4">
                                        <div className="w-16 h-16 rounded-2xl bg-white/[0.05] border border-white/5 overflow-hidden flex-shrink-0">
                ...
                <div className="bg-white/5 rounded-3xl border border-white/5 shadow-soft p-5">
                    <div className="flex items-center gap-3 mb-4">
                        <div className="w-10 h-10 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center justify-center text-content">
                            <MapPin size={20} />
                        </div>
                        <div>
                ...
                {/* ── Need Help ─────────────────────────────── */}
                <button className="w-full h-14 rounded-2xl bg-white/5 border border-white/5 shadow-soft flex items-center justify-center gap-3 active:scale-95 transition-transform group">
