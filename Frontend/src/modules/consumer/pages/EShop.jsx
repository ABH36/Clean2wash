    ...
    return (
        <MobileLayout>
            <div className="bg-slate-50 min-h-screen font-sans pb-32">
                {/* ── Compact Header ── */}
                <header className="sticky top-0 z-[60] bg-white/5 px-5 pt-8 pb-4 border-b border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate('/')} className="w-10 h-10 bg-white/[0.02] rounded-xl flex items-center justify-center active:scale-95 transition-all">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => navigate('/cart')} className="w-10 h-10 bg-slate-50 rounded-xl flex items-center justify-center relative active:scale-95 transition-all">
                            ...
                            {cartCount > 0 && <span className="absolute top-2 right-2 w-4 h-4 bg-brand text-slate-900 text-[8px] font-bold rounded-full flex items-center justify-center border-white/5 border-slate-50">{cartCount}</span>}
                        </button>
                    </div>

                    <AnimatePresence>
                        {isSearching && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="absolute top-full left-0 right-0 bg-white/5 border-b border-white/5 p-5 shadow-2xl shadow-black/50">
                ...
                <div className="space-y-6">
                    ...
                    {!searchQuery && activeCategory === 'All' && (
                        <div className="px-5 pt-6">
                            {(metadata.banners?.length > 0 ? metadata.banners : [ { title: "Flash sale\n10% Off", subtitle: "Grand Launch", cta: "Shop now", theme: 'dark' } ]).map((banner, idx) => (
                                <div key={idx} className={`rounded-[2.5rem] p-8 min-h-[180px] relative overflow-hidden flex flex-col justify-between shadow-2xl shadow-black/50 shadow-slate-900/10 ${banner.theme === 'dark' ? 'bg-slate-900 text-white' : 'bg-brand text-slate-900'}`}>
                                    ...
                                    <button onClick={() => navigate(banner.path || '#')} className="relative z-10 w-fit mt-6 px-6 py-3 bg-white/5 text-slate-900 rounded-2xl font-bold text-[12px] shadow-2xl shadow-black/50 active:scale-95 transition-all">
                                        {banner.cta || 'Explore'}
                                    </button>
                    ...
                    <div className="px-5 flex gap-3">
                        <div className="flex-1 bg-white/5 p-4 rounded-[1.8rem] border border-white/5 flex items-center gap-3 ">
                            ...
                            {['All', ...CATEGORIES_DATA.map(c => c.key)].map((catKey) => {
                                ...
                                return (
                                    <button key={catKey} onClick={() => setActiveCategory(catKey)}
                                        className={`flex-shrink-0 px-6 py-2.5 rounded-full text-[12px] font-bold transition-all border ${isActive ? 'bg-slate-900 border-slate-900 text-white shadow-lg shadow-slate-900/10' : 'bg-white/5 border-slate-100 text-slate-400'}`}>
                                        {cat.title}
                                    </button>
                                );
                            })}
                            ...
                            <div className="flex gap-4 px-5 overflow-x-auto no-scrollbar pb-2">
                                {trendingProducts.map((item, i) => (
                                    <motion.div key={item._id} whileTap={{ scale: 0.99 }} onClick={() => navigate(`/e-shop/product/${item._id}`)}
                                        className="flex-shrink-0 w-64 bg-white/5 rounded-[2rem] p-5 border border-white/5  flex items-center gap-4 group cursor-pointer transition-all hover:border-slate-200">
                    ...
                    <div className="px-5">
                        ...
                        {loading ? (
                            <div className="grid grid-cols-2 gap-4">
                                {[1, 2, 3, 4].map(n => <div key={n} className="h-64 bg-white/5 rounded-[2rem] border border-white/5 animate-pulse" />)}
                            </div>
                        ) : products.length === 0 ? (
                            <div className="py-16 bg-white/5 rounded-[2rem] border border-dashed border-white/10 text-center px-8">
                                ...
                                {products.map((prod, i) => {
                                    ...
                                    return (
                                        <motion.div key={pId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                                            className="bg-white/5 rounded-[2.2rem] p-3 border border-white/5  group">
                                            <div className="relative aspect-square rounded-[1.8rem] overflow-hidden bg-slate-50 mb-3 cursor-pointer" onClick={() => navigate(`/e-shop/product/${pId}`)}>
                                                ...
                                                <button onClick={(e) => { e.stopPropagation(); toggleWishlist(prod); }} className="absolute top-2 right-2 w-8 h-8 bg-white/90 backdrop-blur-md rounded-xl flex items-center justify-center  active:scale-90 transition-all">
                                            ...
                                            <div className="px-1 space-y-2">
                                                ...
                                                {isBlackPassMember && (
                                                    <div className="bg-slate-900 text-white p-1 rounded-lg px-2 w-fit flex items-center gap-1.5 shadow-2xl shadow-black/50 shadow-brand/10 border border-brand/20">
                    ...
                    {fromBooking && (
                        <div className="fixed bottom-24 inset-x-5 z-[70]">
                            <button onClick={() => navigate(fromBooking === 'studio-wash' ? '/full-wash-booking?back=shop' : '/instant-wash?back=shop')}
                                className="w-full h-15 bg-slate-900 text-white rounded-[1.8rem] font-bold text-[13px] flex items-center justify-center gap-3 shadow-2xl shadow-black/50 active:scale-95 transition-all">
                ...
                {/* WhatsApp */}
                {!toast && (
                    <a href="https://wa.me/919876543210" className="fixed bottom-24 right-5 w-12 h-12 bg-[#25D366] rounded-full flex items-center justify-center text-white shadow-2xl shadow-black/50 active:scale-90 transition-all z-[60]">
