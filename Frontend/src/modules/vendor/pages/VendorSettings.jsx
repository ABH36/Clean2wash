    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col lg:flex-row gap-8 pb-24 lg:pb-0">
                    ...
                    {SECTIONS.map((section) => (
                        <button
                            ...
                            className={`flex-shrink-0 lg:flex-shrink flex items-center gap-4 p-5 rounded-[2rem] border transition-all duration-300 text-left relative overflow-hidden group ${activeSection === section.id
                                ? 'bg-surface border-brand shadow-2xl shadow-black/50 shadow-brand/10 ring-1 ring-brand/20'
                                : 'bg-surface/40 border-white/5/5 hover:border-white/5/20'
                                }`}
                        >
                            {activeSection === section.id && (
                                <motion.div layoutId="activeBG" className="absolute left-0 w-1.5 h-full bg-brand" />
                            )}
                            <div className={`w-11 h-11 rounded-2xl flex items-center justify-center transition-all ${activeSection === section.id ? 'bg-brand text-white rotate-6' : 'bg-background text-content-subtle border border-white/5/10'}`}>
                ...
                {/* Settings Form */}
                <div className="flex-1 bg-surface rounded-[3rem] border border-white/5/10 shadow-soft p-6 md:p-10 space-y-10 relative overflow-hidden">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
                        ...
                        {activeSection === 'Profile' && (
                            <button
                                onClick={handleSave}
                                disabled={saving}
                                className="h-12 px-8 bg-brand text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-2xl shadow-black/50 shadow-brand/20 flex items-center justify-center gap-3 hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                            >
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 relative z-10">
                            <div className="bg-background/80 backdrop-blur-sm rounded-[2.5rem] p-8 md:p-12 border border-white/5/10 flex flex-col md:flex-row items-center gap-10 shadow-inner group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="bg-background/40 backdrop-blur-sm border border-white/5/10 rounded-[2.5rem] p-8 shadow-inner group/card">
                                    ...
                                    <div className="aspect-[4/3] bg-surface rounded-[2rem] overflow-hidden border border-white/5/10 group relative shadow-2xl">
                                        ...
                                        <div className="absolute inset-0 bg-brand/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center p-6">
                                            <button
                                                ...
                                                className="w-full h-14 bg-white/5 text-white rounded-2xl text-[10px] font-black uppercase tracking-[0.2em] shadow-2xl flex items-center justify-center gap-3 active:scale-95 transition-all"
                                            >
                                                <Eye size={18} /> Inspect Asset
                                            </button>
                                        </div>
                                    </div>
                                    <div className="mt-6 flex items-center justify-center gap-2 py-3 bg-surface/50 rounded-xl border border-white/5/5">
                                ...
                                <div className="bg-background/40 backdrop-blur-sm border border-white/5/10 rounded-[2.5rem] p-8 shadow-inner flex flex-col justify-between">
                                    <div className="space-y-8">
                                        ...
                                        <div className="space-y-4">
                                            {[
                                                ...
                                                <div key={i} className="flex items-center justify-between p-4 bg-surface/30 rounded-2xl border border-white/5/5">
                                    ...
                                    <div className="relative mt-10">
                                        ...
                                        <label
                                            htmlFor="id-upload"
                                            className="w-full h-14 bg-surface border border-brand/20 rounded-2xl text-[10px] font-black uppercase tracking-widest text-brand hover:bg-brand hover:text-white transition-all flex items-center justify-center cursor-pointer  active:scale-95"
                                        >
                        ...
                        <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700 relative z-10">
                            <div className="bg-background/60 backdrop-blur-sm rounded-[3rem] p-10 md:p-14 border border-white/5/10 relative overflow-hidden group">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="p-8 bg-background/40 backdrop-blur-sm rounded-[2.5rem] border border-white/5/10 shadow-inner group hover:border-brand/30 transition-all">
                        ...
                        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 relative z-10">
                            {[
                                ...
                                <div key={opt.id} className="flex items-center justify-between p-8 bg-background/40 backdrop-blur-sm rounded-[2.5rem] border border-white/5/10 shadow-inner group">
                                    <div className="flex items-center gap-6">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all ${form.notifications?.[opt.id] ? 'bg-brand text-white shadow-lg shadow-brand/20' : 'bg-surface/50 text-content-subtle border border-white/5/5'}`}>
                                    ...
                                    >
                                        <div className={`absolute top-1.5 w-6 h-6 rounded-full bg-white/5 shadow-2xl transition-all duration-500 flex items-center justify-center ${form.notifications?.[opt.id] ? 'left-8.5' : 'left-1.5'}`}>
                            ...
                            <div className="md:col-span-2 space-y-3 group">
                                ...
                                <div className="relative">
                                    <textarea
                                        ...
                                        className="w-full bg-background/60 backdrop-blur-sm border border-white/5/10 rounded-[2rem] p-6 text-sm font-black text-content outline-none focus:border-brand/50 focus:bg-background transition-all resize-none h-40 shadow-inner uppercase tracking-tight leading-relaxed placeholder:text-content-subtle/20"
