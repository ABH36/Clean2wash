...
const CaptainNotifications = () => {
    ...
    const getBg = (type) => {
        switch (type) {
            ...
            default: return isDarkMode ? 'bg-white/5 border-white/10' : 'bg-white/[0.05] border-white/10';
        }
    };
    ...
    return (
        <CaptainLayout hideNav>
            <div className={`min-h-[100dvh] pb-24 transition-colors duration-500 ${isDarkMode ? 'bg-[#0F172A]' : 'bg-white/[0.02]'}`}>
                {/* Header */}
                ...
                <div className="px-4 py-6 space-y-4">
                    {loading ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 opacity-40">
                            <div className={`w-20 h-20 rounded-[2.5rem] flex items-center justify-center mb-6 border-4 border-dashed ${isDarkMode ? 'border-white/10' : 'border-white/10'}`}>
                                <Clock size={32} />
                            </div>
                            ...
                            {notifications.map((notif, index) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className={`relative p-4 rounded-3xl border transition-all duration-300 cursor-pointer ${isDarkMode ? 'bg-[#1E293B] border-white/10 shadow-2xl shadow-black/40' : 'bg-white/5 border-white/5 shadow-2xl shadow-black/50 shadow-gray-200/50'} ${notif.isRead ? 'opacity-60 grayscale-[0.5]' : 'scale-[1.02] border-brand/30 shadow-brand/10'}`}
                                    >
                                        {!notif.isRead && (
                                            <div className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-brand shadow-lg shadow-brand/50 border-white/5 border-white dark:border-[#1E293B]" />
                                        )}
