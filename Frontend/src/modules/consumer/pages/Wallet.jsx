    ...
    return (
        <MobileLayout>
            <div className={`min-h-screen transition-colors duration-300 ${isDarkMode ? 'bg-[#0A0F0D]' : 'bg-[#FAF6EB]'}`}>
                <header className={`px-4 py-6 flex items-center justify-between sticky top-0 z-[60] border-b backdrop-blur-xl transition-colors duration-300 ${
                    isDarkMode ? 'bg-[#0A0F0D]/90 border-white/5' : 'bg-white/90 border-black/5'
                }`}>
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate(-1)} className={`w-10 h-10 border rounded-2xl flex items-center justify-center active:scale-95 transition-all ${
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={fetchWalletData} className={`p-2.5 rounded-xl border active:scale-75 transition-all ${
                            isDarkMode ? 'bg-white/[0.03] text-white/40 border-white/10' : 'bg-black/[0.03] text-black/40 border-black/10'
            ...
            <div className="px-4 pb-24 space-y-4 pt-4">
                <AnimatePresence mode="wait">
                        {loading && transactions.length === 0 ? (
                            <div className="animate-pulse space-y-4">
                                <div className={`h-44 rounded-[28px] relative overflow-hidden border ${isDarkMode ? 'bg-[#0F1412] border-white/5' : 'bg-black/05 border-black/5'}`}>
                                     <div className={`absolute inset-0 shimmer-effect ${isDarkMode ? 'opacity-10' : 'opacity-20'}`} />
                                </div>
                                <div className="space-y-2">
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className={`h-20 rounded-[22px] border shimmer-effect ${isDarkMode ? 'bg-white/[0.03] border-white/5 opacity-50' : 'bg-black/[0.02] border-black/5 opacity-40'}`} />
                                    ))}
                                </div>
                            </div>
                        ) : (
                            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className={`relative rounded-[28px] overflow-hidden shadow-2xl border transition-all duration-300 ${
                                isDarkMode ? 'border-white/5 shadow-black/40' : 'bg-white border-black/5 shadow-black/5'
                            }`}>
                                ...
                                <div className="absolute top-[-20%] right-[-10%] w-48 h-48 bg-[#F59E0B]/15 blur-[50px] rounded-full" />
                                 <div className="relative z-10 p-6">
                                    <div className="flex justify-between items-start mb-6">
                                        ...
                                        <div className={`w-11 h-11 rounded-[18px] flex items-center justify-center border shadow-inner ${
                                    ...
                                    <div className="flex gap-2.5">
                                        ...
                                        <button onClick={() => { setWithdrawMode(true); setAddMode(false); }}
                                            className={`flex-1 h-12 rounded-[18px] font-black text-[11px] tracking-tight flex items-center justify-center gap-2 active:scale-95 transition-all border ${
                                                isDarkMode ? 'bg-white/10 text-white border-white/10' : 'bg-black/[0.03] text-black border-black/10'
                                            }`}>
                ...
                <AnimatePresence mode="wait">
                    {addMode && (
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className={`rounded-[28px] p-6 border shadow-2xl transition-all duration-300 ${
                                isDarkMode ? 'bg-white/[0.03] border-white/5 shadow-black/50' : 'bg-white border-black/5 shadow-black/5'
                            } space-y-5`}>
                                <div className="flex justify-between items-center">
                                    <h3 className={`text-[14px] font-[1000] uppercase tracking-tight ${isDarkMode ? 'text-white' : 'text-black'}`}>Deposit funds</h3>
                                    <button onClick={() => setAddMode(false)} className={`w-8 h-8 rounded-lg flex items-center justify-center active:scale-90 ${isDarkMode ? 'bg-white/5 text-white/40' : 'bg-black/05 text-black/40'}`}><X size={16} /></button>
                                </div>
                                <div className="grid grid-cols-4 gap-2">
                                    {QUICK_AMOUNTS.map((amt) => (
                                        <button key={amt} onClick={() => setSelectedAmt(amt)} 
                                            className={`h-11 rounded-xl text-[10px] font-black tracking-widest transition-all uppercase ${
                                                selectedAmt === amt 
                                                    ? 'bg-[#F59E0B] text-black shadow-2xl shadow-[#F59E0B]/20 transform -translate-y-0.5' 
                                                    : (isDarkMode ? 'bg-white/5 text-white/40 border border-white/5' : 'bg-black/05 text-black/40 border border-black/5')
                                            }`}>
                                            ₹{amt}
                                        </button>
                                    ))}
                                ...
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="number" placeholder="0.00" value={selectedAmt || ''} onChange={(e) => setSelectedAmt(Number(e.target.value))}
                                            className={`w-full h-12 rounded-[18px] px-10 font-[1000] outline-none transition-all border ${
                                                isDarkMode ? 'bg-white/[0.03] border-white/10 text-white focus:border-[#F59E0B]/40 placeholder:text-white/10' : 'bg-black/[0.02] border-black/10 text-black focus:border-[#F59E0B]/40 placeholder:text-black/20'
                                            }`} />
                                    </div>
                                </div>
                                <button onClick={handleAddMoney} disabled={loading || !selectedAmt} className={`w-full h-14 rounded-[18px] font-black text-[12px] uppercase tracking-[0.2em] flex items-center justify-center gap-3 active:scale-95 shadow-2xl disabled:opacity-30 ${
                                    isDarkMode ? 'bg-white text-black shadow-black/50' : 'bg-black text-white shadow-black/20'
                                }`}>
                        ...
                        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                            <div className={`rounded-[28px] p-6 border shadow-2xl transition-all duration-300 ${
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <button onClick={() => setWithdrawMode(false)} className={`w-8 h-8 rounded-lg flex items-center justify-center active:scale-90 ${isDarkMode ? 'bg-white/5 text-white/40' : 'bg-black/05 text-black/40'}`}><X size={16} /></button>
                                </div>
                                <div className="space-y-1.5">
                                    <p className={`text-[8px] font-black uppercase tracking-widest ml-1 ${isDarkMode ? 'text-white/20' : 'text-black/30'}`}>Payout Volume</p>
                                    <div className="relative">
                                        <div className={`absolute left-4 top-1/2 -translate-y-1/2 font-black tracking-widest ${isDarkMode ? 'text-white/20' : 'text-black/20'}`}>₹</div>
                ...
                <div onClick={() => navigate('/refer-earn')} className={`group rounded-[24px] p-4 flex items-center gap-4 border cursor-pointer active:scale-[0.98] transition-all relative overflow-hidden ${
                    isDarkMode ? 'bg-white/[0.03] border-white/5 hover:border-[#F59E0B]/20' : 'bg-white border-black/5 shadow-sm'
                }`}>
                    <div className="absolute top-[-50%] right-[-10%] w-32 h-32 bg-[#F59E0B]/5 blur-[30px] rounded-full pointer-events-none" />
                    <div className={`w-11 h-11 rounded-[16px] flex items-center justify-center text-[#F59E0B] border relative shadow-inner ${
                        isDarkMode ? 'bg-white/5 border-white/10' : 'bg-black/[0.03] border-black/5'
                    }`}>
                    ...
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
                        isDarkMode ? 'bg-white/5 text-white/20 group-hover:text-[#F59E0B] group-hover:bg-[#F59E0B]/10' : 'bg-black/5 text-black/20 group-hover:text-[#F59E0B] group-hover:bg-[#F59E0B]/05'
                    }`}>
                        ...
                        {loading && transactions.length === 0 ? (
                            ...
                            transactions.map((txn, idx) => (
                                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }}
                                    key={txn._id} className={`rounded-[22px] p-4 border flex items-center gap-4 transition-all ${
                                        isDarkMode ? 'bg-white/[0.03] border-white/5 hover:bg-white/[0.05]' : 'bg-white border-black/5 hover:bg-black/[0.01] shadow-sm'
                                    }`}>
