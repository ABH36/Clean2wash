            ...
            {isVisible && (
                ...
                >
                    <motion.div
                        ...
                        className={`w-full max-w-md rounded-[3rem] overflow-hidden shadow-2xl relative transition-colors duration-500 ${isDarkMode ? 'bg-[#1E293B] border border-white/5' : 'bg-white'}`}
                    >
                        {/* Header Gradient */}
                        ...
                        <div className="px-8 -mt-6 relative z-20">
                            <div className={`rounded-[2.5rem] p-6 shadow-xl space-y-6 transition-colors duration-500 ${isDarkMode ? 'bg-[#2A3547] border border-white/5' : 'bg-white'}`}>
                                <div className="flex items-center gap-4">
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center overflow-hidden border transition-colors ${isDarkMode ? 'bg-white/5 border-white/5' : 'bg-gray-50 border-gray-100'}`}>
                        ...
                        <div className="px-8 pt-8 pb-10 flex gap-4">
                            <button
                                onClick={onReject}
                                className={`flex-1 py-4 rounded-2xl font-black text-xs uppercase tracking-widest border transition-all ${isDarkMode ? 'bg-white/5 border-white/5 text-white/40 active:bg-white/10' : 'bg-gray-50 border-gray-100 text-content-muted active:bg-gray-100'}`}
                            >
                                Pass
                            </button>
                            <motion.button
                                whileTap={{ scale: 0.96 }}
                                onClick={onAccept}
                                className="flex-[2] py-4 rounded-2xl bg-brand text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-brand/30 flex items-center justify-center gap-2 hover:bg-brand-light transition-all"
                            >
