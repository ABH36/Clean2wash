...
const BottomNav = ({ NAV_GROUPS, location, navigate, setIsMobileDrawerOpen }) => {
    // Pick 4 key items for bottom nav
    const BOTTOM_ITEMS = [
        { ...NAV_GROUPS[0].items[0], label: 'Home' },      // Dashboard
        { ...NAV_GROUPS[0].items[1], label: 'Orders' },    // Orders
        { ...NAV_GROUPS[1].items[1], label: 'Shop' },      // Products
        { ...NAV_GROUPS[2].items[1], label: 'Earnings' },  // Earnings
    ];

    return (
        <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[#0f1117]/95 backdrop-blur-xl border-t border-white/5 px-2 py-3 flex justify-around items-center z-[100] shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            {BOTTOM_ITEMS.map((item) => {
                ...
                return (
                    <button
                        ...
                        className={`flex flex-col items-center gap-1.5 px-3 py-1 rounded-2xl transition-all relative ${isActive ? 'text-brand' : 'text-white/30'}`}
                    >
                        {isActive && (
                            <motion.div
                                layoutId="bottomNavActive"
                                className="absolute -top-3 left-1/2 -translate-x-1/2 w-8 h-1 bg-brand rounded-full shadow-[0_0_10px_rgba(244,117,33,0.5)]"
                            />
                        )}
            ...
            <button
                ...
                className="flex flex-col items-center gap-1.5 px-3 py-1 text-white/30"
            >
                <Menu size={20} />
...
const VendorLayout = ({ children, title, subtitle }) => {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col md:flex-row font-sans transition-colors duration-500 overflow-x-hidden">
                        ...
                        >
                            <div className="p-6 flex items-center justify-between border-b border-white/5">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-sm font-black uppercase tracking-tighter">CarWash <span className="text-brand">Hub</span></span>
                                </div>
            ...
            <main className="flex-1 overflow-y-auto relative h-screen">
                ...
                <header className="bg-surface/80 backdrop-blur-xl border-b border-gray-100/10 px-4 md:px-6 py-3 md:py-4 flex items-center justify-between sticky top-0 z-50 transition-colors duration-500">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-sm md:text-lg font-black tracking-tight text-content leading-none">{title}</h1>
                            <div className="flex items-center gap-2 mt-1">
                                <p className="text-[8px] md:text-[10px] text-content-subtle font-bold uppercase tracking-widest">{subtitle}</p>
                                <div className="w-1 h-1 bg-gray-300 rounded-full hidden md:block" />
                                <LocationIndicator variant="minimal" className="!p-0 !bg-transparent !border-none hidden md:flex" />
                            </div>
                        </div>
                    </div>
                ...
                <div className="p-4 md:p-10 max-w-7xl mx-auto relative pb-24 md:pb-10">
                                        ...
                                        <div className="grid grid-cols-2 gap-4 w-full">
                                            <div className="bg-background p-4 rounded-3xl border border-gray-100/10">
                                                ...
                                                <div className="flex items-center gap-2 justify-center">
                                                    ...
                                                    <span className={`text-[10px] font-black uppercase tracking-tighter ${user.verificationStatus === 'rejected' ? 'text-red-500' : 'text-brand'}`}>
