...
const OperationsSection = ({
    ...
    <div className="bg-white dark:bg-slate-900 border border-black/[0.05] dark:border-white/10 rounded-[1rem] overflow-hidden shadow-[0_14px_30px_rgba(15,23,42,0.06)] dark:shadow-[0_18px_36px_rgba(0,0,0,0.45)]">
        <div className="px-4 py-3.5 border-b border-gray-100 dark:border-white/10 flex items-center justify-between gap-3 flex-wrap">
            <div>
                <p className="text-[9px] font-black text-black/30 dark:text-white/45 uppercase tracking-widest mb-1">Live chauffeur ops</p>
                <h3 className="text-[14px] font-black text-black dark:text-white uppercase">Active Trips (Essential View)</h3>
            </div>
            <div className="flex items-center gap-3 flex-wrap">
                <span className="h-9 px-3 inline-flex items-center rounded-lg bg-gray-100 dark:bg-white/10 text-[10px] font-black text-black/60 dark:text-white/70 uppercase">
                    {filteredLiveBookings.length} Live
                </span>
                <select
                    value={opsFilter}
                    onChange={(event) => setOpsFilter(event.target.value)}
                    className="h-9 border border-gray-200 dark:border-white/15 bg-white dark:bg-slate-800 rounded-lg px-3 text-[11px] font-black text-black dark:text-white uppercase outline-none"
                >
                ...
                <input
                    ...
                    className="h-9 w-64 max-w-full border border-gray-200 dark:border-white/15 bg-white dark:bg-slate-800 rounded-lg px-3 text-[11px] font-bold text-black dark:text-white outline-none focus:border-black dark:focus:border-white/50"
                />
            </div>
        </div>
        ...
        {bookingsLoading ? (
            ...
            <div className="py-16 text-center">
                ...
                <p className="text-[10px] font-black text-black/20 dark:text-white/35 uppercase tracking-widest">No chauffeur trips match this filter</p>
            </div>
        ) : (
                        ...
                        <div key={booking._id} className="px-4 py-4 grid grid-cols-1 xl:grid-cols-[1.35fr_1fr_auto] gap-3 items-center">
                            ...
                            <div className="xl:justify-self-end">
                                <button
                                    ...
                                    className="h-9 px-4 bg-black dark:bg-white text-white dark:text-black text-[10px] font-black uppercase rounded-lg hover:bg-brand hover:text-black transition-colors"
                                >
                                    Manage trip
                                </button>
                            </div>
