...
subscriptionSchema.index({ consumer: 1, status: 1 });
subscriptionSchema.index({ endDate: 1 });
subscriptionSchema.index({ plan: 1 });

// Static method to get active subscription
subscriptionSchema.statics.getActiveSubscription = async function(consumerId) {
    return this.findOne({
        consumer: consumerId,
        status: 'active',
        endDate: { $gt: new Date() }
...
subscriptionSchema.statics.createSubscription = async function(subscriptionData) {
    ...
    // Update consumer with subscription reference
    await mongoose.model('Consumer').findByIdAndUpdate(
        subscriptionData.consumer,
