...
const VehicleManager = () => {
    ...
    const renderTypeSelection = () => (
        <div className="px-6 py-8">
            <h2 className="text-3xl font-[1000] text-white tracking-tight mb-2">Choose Your<br />Vehicle Type</h2>
            <p className="text-sm font-medium text-white/30 mb-10">You can add more vehicles later from the home screen</p>
            ...
            <div className="grid grid-cols-2 gap-4">
                {[
                    ...
                    <motion.button
                        ...
                        className="relative h-[180px] bg-white/[0.02] border border-white/5 rounded-[2rem] overflow-hidden  group"
                    >
                        <div className="absolute inset-0 p-5 z-10 flex flex-col justify-start text-left">
                            ...
                            <span className="text-white text-[13px] font-[1000] leading-tight max-w-[80px]">{item.title}</span>
                        </div>
                        <img src={item.image} className="absolute bottom-0 right-[-10%] w-[120%] h-[70%] object-contain grayscale group-hover:grayscale-0 transition-all duration-700" alt="" />
                    </motion.button>
                ))}
            </div>
        </div>
                ...
                <div className="px-6 mb-6">
                    <div className="relative mb-8">
                        ...
                        <input 
                            ...
                            className="w-full h-14 pl-12 pr-4 bg-white/[0.02] rounded-2xl text-[13px] font-bold text-white border border-black/05 outline-none focus:border-[#F59E0B]/20 transition-all"
                        />
                    </div>

                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-[11px] font-black text-white/20 uppercase tracking-[0.2em]">Popular Brands</h3>
                    ...
                    <div className="grid grid-cols-3 gap-y-10 gap-x-4">
                        {loading && brands.length === 0 ? (
                            <div className="col-span-3 py-10 flex flex-col items-center">
                                ...
                                <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Loading Brands...</p>
                            </div>
                            ...
                            filteredBrands.map(brand => (
                                ...
                                >
                                    <div className="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center border border-black/05  mb-3 group-hover:border-[#F59E0B]/20 transition-all p-3 overflow-hidden">
                                        {BRAND_LOGOS[brand.toUpperCase()] ? (
                                            ...
                                            <span className="text-[10px] font-black text-white/40 uppercase">{brand.slice(0, 3)}</span>
                                        )}
                                    </div>
                                    <span className="text-[10px] font-black text-white/60 uppercase tracking-tighter text-center">{brand}</span>
                                </motion.button>
                            ))
                        ) : (
                            <div className="col-span-3 py-10 flex flex-col items-center">
                                <p className="text-[10px] font-bold text-white/40 tracking-widest">No brands found</p>
                                <button 
    ...
    const renderModelSelection = () => (
        <div className="px-0 py-4 animate-in fade-in slide-in-from-bottom-4">
            <div className="px-6 mb-6">
                <h2 className="text-2xl font-[1000] text-white tracking-tight mb-1">Vehicles by</h2>
            ...
            <div className="grid grid-cols-3 gap-y-10 gap-x-4 px-6 pb-20">
                {models.map(m => (
                    <motion.button
                        ...
                        className="flex flex-col items-center group"
                    >
                        <div className="w-20 h-20 bg-white/[0.03] rounded-2xl flex items-center justify-center relative overflow-hidden mb-3 border border-white/5 group-hover:bg-white/5 transition-all">
                            <img src={m.image} className="w-[85%] h-[85%] object-contain group-hover:scale-110 transition-transform duration-500" alt={m.model} />
                        </div>
                        <span className="text-[10px] font-black text-white/80 uppercase tracking-tight text-center leading-tight truncate w-full">{m.model}</span>
                    </motion.button>
                ))}
                ...
                <motion.button
                    ...
                    className="flex flex-col items-center group"
                >
                    <div className="w-20 h-20 bg-white/[0.03] rounded-2xl flex items-center justify-center border border-dashed border-white/10 mb-3 group-hover:bg-white/5 transition-all text-white/20">
                        <Plus size={24} />
                    </div>
                    <span className="text-[10px] font-black text-white/40 uppercase tracking-tight text-center">Others</span>
                </motion.button>
            </div>
        </div>
    );
        ...
        <div className="px-6 py-4 animate-in fade-in slide-in-from-bottom-4">
             <div className="mb-4 p-4 bg-black rounded-[2rem] relative overflow-hidden shadow-2xl">
                <div className="absolute right-[-10%] top-0 h-full w-[60%] opacity-20 transform skew-x-[-20deg]">
                    <img src={selectedModel?.image} className="h-full w-full object-cover" alt="" />
                </div>
                ...
                <div className="absolute top-1/2 -translate-y-1/2 right-4 w-28 h-28">
                    <img src={selectedModel?.image} className="w-full h-full object-contain drop-shadow-2xl" alt="" />
                </div>
            </div>

            <div className="space-y-4">
                <div className="space-y-2">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1">Select Color</label>
                ...
                <div className="space-y-1">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1">Vehicle Model</label>
                    <input 
                        ...
                        className={`w-full h-14 bg-white/[0.03] border border-white/10 rounded-2xl px-6 font-black text-base text-white uppercase tracking-[0.1em] outline-none focus:border-[#F59E0B]/30  transition-all ${selectedModel?.model !== 'Custom Model' ? 'opacity-50' : ''}`} 
                    />
                </div>

                <div className="space-y-1">
                    <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1">Plate Number</label>
                    <input 
                        placeholder="EX: KA05M1234" 
                        value={form.plate} 
                        onChange={e => setForm({...form, plate: e.target.value.toUpperCase()})}
                        className={`w-full h-14 bg-white/[0.03] border ${errors.plate ? 'border-rose-500/50' : 'border-white/10'} rounded-2xl px-6 font-black text-base text-white uppercase tracking-[0.1em] outline-none focus:border-[#F59E0B]/30  transition-all`} 
                    />
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1">Insurance Expiry</label>
                        <input 
                            type="date" 
                            value={form.insuranceExpiry} 
                            onChange={e => setForm({...form, insuranceExpiry: e.target.value})}
                            className="w-full h-12 bg-white/[0.03] border border-white/10 rounded-2xl px-4 font-bold text-[12px] text-white outline-none focus:border-[#F59E0B]/30" 
                        />
                    </div>
                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-white/20 uppercase tracking-[0.2em] ml-1">PUC Expiry</label>
                        <input 
                            type="date" 
                ...
                <div className="p-3 bg-white/[0.03] rounded-2xl border border-white/5 flex items-center gap-4">
                    <div className="w-8 h-8 bg-white/5 rounded-xl flex items-center justify-center text-[#F59E0B]  flex-shrink-0">
                        <Info size={16} />
                    </div>
                    <div>
                        <h4 className="text-[9px] font-black text-white uppercase tracking-tight">Verify vehicle</h4>
                        <p className="text-[8px] font-medium text-white/30 leading-tight">Match details with your registration documents.</p>
                    </div>
                </div>
    ...
    const renderList = () => (
        <div className="px-6 pt-4 space-y-6">
            <div className="flex items-center justify-between mb-2">
                <h2 className="text-[11px] font-black text-white/20 uppercase tracking-[0.3em]">Active Garage</h2>
                <span className="text-[9px] font-black text-white/40">{vehicles?.length} Active Assets</span>
            </div>

            {vehicles?.map((v) => (
                <motion.div 
                    key={v._id || v.id} 
                    className={`bg-white/5 rounded-[2.5rem] border overflow-hidden relative transition-all group ${v.isPrimary ? 'border-[#F59E0B]/30 shadow-2xl shadow-black/5' : 'border-white/5 '}`}
                >
                    <div className="relative h-48 bg-white/[0.02] flex items-center justify-center p-8">
                        <img src={v.image || "https://images.unsplash.com/photo-1494905998402-395d579af36f?auto=format&fit=crop&q=80&w=600"} className="w-full h-full object-contain group-hover:scale-110 transition-transform duration-700" alt="" />
                        {v.isPrimary && (
                        ...
                        <div className="absolute bottom-6 right-6 flex gap-2">
                            <button onClick={() => removeVehicle(v._id || v.id)} className="w-10 h-10 bg-white/5 shadow-2xl shadow-black/50 rounded-2xl flex items-center justify-center text-rose-500 active:scale-90 transition-all border border-black/05"><Trash2 size={18} /></button>
                             {!v.isPrimary && (
                    ...
                    <div className="px-8 py-6 flex items-center justify-between">
                        <div>
                            ...
                            <h4 className="text-white text-2xl font-[1000] tracking-tighter leading-none mt-1">{v.model}</h4>
                        </div>
                        <div className="bg-white/5 border border-white/5 px-4 py-2 rounded-xl">
                            <span className="text-white text-[12px] font-black tracking-widest uppercase">{v.plate || v.regNo}</span>
                        </div>
                    </div>
                </motion.div>
            ))}
            ...
            <button 
                ...
                className="w-full h-16 border-white/5 border-dashed border-black/05 rounded-[2.5rem] flex items-center justify-center gap-4 text-white/20 hover:text-white/40 hover:border-black/10 transition-all active:scale-[0.98]"
            >
                <div className="w-10 h-10 bg-white/5 rounded-2xl flex items-center justify-center "><Plus size={20} /></div>
    ...
    return (
        <MobileLayout>
            <div className="min-h-screen bg-[#0A0F0D] font-sans pb-32">
                <header className="px-6 py-5 flex items-center justify-between bg-[#0A0F0D]/90 backdrop-blur-xl sticky top-0 z-[60] border-b border-white/5">
                    <div className="flex items-center gap-4">
                        <button 
                            ...
                            className="w-10 h-10 bg-white/[0.03] rounded-2xl flex items-center justify-center active:scale-95 transition-all border border-white/10"
                        >
                            <ChevronLeft size={20} className="text-white" />
                        </button>
                        <div>
                             <h1 className="text-xl font-[1000] text-white tracking-tight leading-none">
