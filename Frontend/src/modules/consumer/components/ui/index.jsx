            ...
            {open && (
                <>
                    ...
                    <motion.div initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }}
                        ...
                        className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-white/5 rounded-t-3xl z-50 overflow-hidden"
                        ...
                        {title && (
                            <div className="flex items-center justify-between px-5 py-3 border-b border-white/5">
                                ...
                                <button onClick={onClose} className="w-8 h-8 bg-white/[0.02] rounded-xl flex items-center justify-center text-content-muted">
        ...
        <AnimatePresence>
            {visible && (
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                    className={`fixed top-14 left-1/2 -translate-x-1/2 z-[200] flex items-center gap-2.5 ${s.bg} text-white px-4 py-3 rounded-xl shadow-2xl shadow-black/50`}>
                    {s.icon}
...
export const SearchBar = ({ placeholder = 'Search…', value, onChange, onClear, className = '' }) => (
    <div className={`flex items-center gap-3 bg-white/[0.02] border border-white/5 rounded-xl px-4 py-3 ${className}`}>
...
export const Badge = ({ children, variant = 'brand', className = '' }) => {
    const variants = {
        ...
        yellow: 'bg-accent-yellow text-white border-yellow-300',
    };
    return (
...
export const EmptyState = ({ icon, title, desc, cta, onCta }) => (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
        <div className="w-16 h-16 bg-white/[0.02] border border-white/5 rounded-2xl flex items-center justify-center mx-auto mb-4 text-content-subtle">
        ...
        {cta && (
            <motion.button whileTap={{ scale: 0.97 }} onClick={onCta}
                className="mt-5 bg-brand text-white px-5 py-2.5 rounded-xl font-black text-sm shadow-2xl shadow-black/40 shadow-brand/25">
                {cta}
            </motion.button>
        )}
    </div>
    ...
    <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {items.map(item => (
            <button key={item} onClick={() => onChange(item)}
                className={`flex-shrink-0 px-3.5 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest border transition-all ${active === item ? 'bg-brand text-white border-brand shadow-2xl shadow-black/40' : 'bg-white/5 text-content-muted border-white/5 hover:border-brand/20'
                    }`}>
                {item}
            </button>
        ))}
