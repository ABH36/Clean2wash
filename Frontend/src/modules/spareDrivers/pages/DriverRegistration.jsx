...
const DriverRegistration = () => {
    ...
    useEffect(() => {
        ...
        spareDriverAPI.getProfile()
            .then((res) => {
                ...
                if (status === DRIVER_STATUS.PENDING_DOCS) setStep(STEPS.UPLOAD_DOCS);
                else if (status === DRIVER_STATUS.PENDING_VERIFICATION) setStep(STEPS.VERIFYING);
                else if (status === DRIVER_STATUS.ACTIVE) setStep(STEPS.SUCCESS);
            })
            .catch(() => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            const res = await spareDriverAPI.login({
                phone: form.phone,
            ...
            else setError('Your account is not ready yet. Please contact support.');
        } catch (err) {
            setError(err.message);
        } finally {
            setLoading(false);
            ...
            <div className="min-h-screen bg-white px-5 pt-12 pb-10" style={{ maxWidth: 430, margin: '0 auto' }}>
                <div className="mb-8">
                    ...
                    <h1 className="text-2xl font-black text-black uppercase tracking-tight leading-tight">
                        {authMode === 'register' ? 'Driver Registration' : 'Driver Login'}
                    </h1>
                    <p className="text-[10px] font-black text-black/30 uppercase tracking-widest mt-1">
                        {authMode === 'register' ? 'Create your account to get started' : 'Sign in to manage your chauffeur trips'}
                    </p>
                </div>

                <div className="grid grid-cols-2 gap-2 p-1 mb-5 bg-gray-50 rounded-md border border-gray-100">
                    {[
                        ...
                        <button
                            ...
                            className={`h-10 rounded-md text-[10px] font-black uppercase tracking-widest transition-all ${
                                authMode === mode.id ? 'bg-black text-white' : 'text-black/40'
                            }`}
                        >
                            {mode.label}
                        </button>
                ...
                <form onSubmit={authMode === 'register' ? handleRegister : handleLogin} className="space-y-4">
                    ...
                    >
                        {loading ? (
                            ...
                            <>
                                {authMode === 'register' ? 'Proceed to Documents' : 'Open Driver Account'}
                ...
                <p className="text-center text-[9px] font-black text-black/20 uppercase tracking-widest mt-8">
                    {authMode === 'register' ? 'Already a driver?' : 'Need a new driver account?'}{' '}
                    <button
                        onClick={() => {
                    ...
                    >
                        {authMode === 'register' ? 'Sign In Here' : 'Register Now'}
                    </button>
                </p>
            </div>
        );
        ...
        return (
            <div className="min-h-screen bg-white px-5 pt-12 pb-10" style={{ maxWidth: 430, margin: '0 auto' }}>
                ...
                <div className="space-y-3">
                    <DocUpload label="Aadhaar Card" icon={FileText} file={docs.aadhaarCard} onChange={(e) => setDocs({ ...docs, aadhaarCard: e.target.files?.[0] || null })} />
                    <DocUpload label="Driving License" icon={MapPin} file={docs.drivingLicense} onChange={(e) => setDocs({ ...docs, drivingLicense: e.target.files?.[0] || null })} />
                    <DocUpload label="Live Selfie" icon={Camera} file={docs.selfie} onChange={(e) => setDocs({ ...docs, selfie: e.target.files?.[0] || null })} />
                </div>
